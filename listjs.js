      ...
      try {
        ...
        if (distances.length === 0) { // Check if the length of distances is 0
          ...
          distances.forEach(item => { // Loop through the distances list
            ...
            listItem.innerHTML = `<span class="donor-name">${item.donor.name}</span> - ${item.donor.bloodType} -  ${item.donor.contact} -  ${item.district}  -  ${item.distance} km <div class="address">${item.donor.address}</div>`; // Display the donor's name, blood type, contact, address and distance for each donor according to the distance
